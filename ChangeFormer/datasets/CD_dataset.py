...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, root_dir, split='train', img_size=256, is_train=True,to_tensor=True):
        ...
        self.list_path = os.path.join(root_dir, LIST_FOLDER_NAME, self.split+'.txt')
...
class CDDatasetTest(ImageDataset):

    def __init__(self, root_dir, img_size, split='train', is_train=True, to_tensor=True):
        super(CDDatasetTest, self).__init__(root_dir, img_size=img_size, split=split, is_train=is_train,
                                        to_tensor=to_tensor)
    ...
    def __getitem__(self, index):
        ...
        width = torch.ShortTensor([img.shape[1]])
        [img, img_B] = self.augm.transform_test([img, img_B], to_tensor=self.to_tensor)
        # print(label.max())

        return {'name': name, 'A': img, 'B': img_B, 'W': width}
